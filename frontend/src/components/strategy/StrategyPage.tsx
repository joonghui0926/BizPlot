...
import {
  ...
  TrendingUp, ArrowRight, Check, CircleDollarSign, BarChart2, ChevronRight
} from "lucide-react"
...
import type { Dashboard, Store as StoreType, Action, ActionPlan, Simulation } from "@/lib/types"
                ...
                {actions.map(action => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative w-full text-left flex items-start gap-3 p-4 rounded-2xl border bg-white transition-all ${
                        isSelected
                    ...
                    >
                      ...
                      <span className="flex-1 min-w-0">
                        ...
                        <span className="block text-[11.5px] text-slate-400 mt-1 leading-relaxed line-clamp-2">{action.description}</span>
                  ...
                  <>
                    ...
                    <div className="border-b border-slate-100 py-6">
                      ...
                      <div className="divide-y divide-slate-100">
                        {actions.map(action => (
                          <ActionCard key={action.id} action={action} checked={selected.has(action.id)} onToggle={() => toggle(action.id)} />
                        ))}
                      </div>
                    </div>
...
function ActionCard({ action, checked, onToggle }: { action: Action; checked: boolean; onToggle: () => void }) {
  const meta = ACTION_META[action.type] ?? ACTION_META.operation
  const impact = action.expected_impact
  ...
  return (
    <div className="relative flex items-start gap-3 py-4">
      ...
      <div className="pl-4 flex-1 min-w-0">
        ...
        <div className="text-[11.5px] text-slate-400 mt-0.5 leading-relaxed line-clamp-2">{action.description}</div>
        {(impact.cash_runway_days_delta || impact.revenue_change_pct) && (
